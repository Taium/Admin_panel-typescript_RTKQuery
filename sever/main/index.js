...
app.get("/getall", function (req, res) {
  dbConn.query("SELECT category.name as cat_name, subcategoey.name as subcat_name,products.id as p_id, products.name as p_name , products.price , products.available_qty FROM ((category INNER JOIN subcategoey ON category.id= subcategoey.cid) INNER JOIN products ON subcategoey.id = products.scid)", function (error, results, fields) {
    if (error) throw error;
...
app.put("/updateall/:id",function(req,res){
    ...
    if( pname){
    dbConn.query('UPDATE products,category,subcategoey  set category.name=?,subcategoey.name=?, products.name=?, products.price=?, products.available_qty=? WHERE category.id= subcategoey.cid AND subcategoey.id = products.scid AND products.id=?',[catname,scatname,pname,price,available_qty,id],function(err,results){
        if(err){throw err}
        let message = "";
...
app.delete("/scatdelete/:id", function (req, res) {
  ...
  dbConn.query("DELETE FROM subcategoey where id=?", id, function (err, results) {
    if (err) {
      throw err;
    }
    let message = "";
...
app.put("/updateproduct/:id",function(req,res){
    ...
    let available_qty = req.body.available_qty
    if(name && price && id ){
    dbConn.query('UPDATE products set name=?, price=?, available_qty=? WHERE id=?',[name,price,available_qty,id],function(err,results){
        if(err){throw err}
        let message = "";
        if(results.changedRows == 0){
            message = "please input";
